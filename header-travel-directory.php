...
if(is_tax('tribe_events_cat')){
	$event_cat_id = get_queried_object()->term_id;
	 if (function_exists('z_taxonomy_image_url')){
		$imageUrl = z_taxonomy_image_url($event_cat_id); 	                        	
	}

}
$event_img = get_field('upload_events_image','option'); ?>

<style>
.post-type-archive-tribe_events .hero.small-hero{
	background-image: url(<?php if(!empty($imageUrl)){ echo $imageUrl; }else{ echo $event_img; } ?>);
}
</style>
			...
			<div class="container-fluid collapse_md" id="navbar-main-container">
				...
				<nav class="navbar-collapse collapse" id="navbar-main">
                    <ul class="nav navbar-nav cart-nav">
                        ...
                        <li class="bookTrip"><a href="https://www.tripster.com/" target="_blank">Book a Trip</a></li>
					...
					?>
                    <div class="btnBox"><button type="button" class="btn signupBtn create-tripster-modal-btn">Sign up</button></div>
				</nav>
			</div><!-- /.container-fluid -->
		</header>
	</div><!-- /.navbar-wrapper -->
